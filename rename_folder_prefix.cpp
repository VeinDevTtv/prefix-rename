...
void rename_folder_prefix(const std::string& base_dir, const std::string& old_prefix, const std::string& new_prefix, bool dry_run, bool ignore_case) {
    try {
            ...
            if (entry.is_directory()) {
                ...
                if (folder_name_cmp.rfind(old_prefix_cmp, 0) == 0) { // starts with old_prefix
                    ...
                    if (fs::exists(new_path)) {
                        std::cerr << "Warning: Target path already exists, skipping rename: " << new_path << '\n';
                        continue;
                    }
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cerr << "Usage: " << argv[0] << " <directory> <old_prefix> <new_prefix> [--dry-run|-n] [--ignore-case|-i]\n";
        return 1;
    }
